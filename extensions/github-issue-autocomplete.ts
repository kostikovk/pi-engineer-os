...
export default function githubIssueAutocompleteExtension(pi: ExtensionAPI): void {
	pi.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(pi, ctx.cwd);
		if (!resolvedRepo.ok) {
			return; // Silently skip if not a git/GitHub repo
		}
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				...
				if (result.code !== 0) {
					if (!loadErrorShown) {
						loadErrorShown = true;
						// Only notify if gh is available but failed
					}
					return undefined;
				}
